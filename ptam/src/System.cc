...
System::System() :
	...
	pub_pose_ = nh_.advertise<geometry_msgs::PoseWithCovarianceStamped>("pose",
			1);
	pub_pose_world_ = nh_.advertise<geometry_msgs::PoseWithCovarianceStamped>(
			"pose_world", 1);
	pub_info_ = nh_.advertise<ptam_com::ptam_info>("info", 1);
	srvPC_ = nh_.advertiseService("pointcloud", &System::pointcloudservice,
			this);
	...
	if (topic == "/image") {
		ROS_WARN(
				"video source: image has not been remapped! Typical command-line usage:\n"
...
//    }
		...
		//Get the accumulated delta imu values uptil now
		imu_orientation = imu_rotation_;
		//And reset the imu_rotation_ matrix
		imu_rotation_ = TooN::Identity(3);
	}
...
//    imu_msgs_.pop();
	...
	//@KSS: Received new delta rotation matrix, accumulate it till we require it next
	/**
	 * M2i = Cij · M1ij
	 Where: M1 is the orientation matrix at the beginning of the calculation cycle.
	 M2 is the orientation matrix at the end of the calculation cycle.
	 *
	 */
	if ((*msg).header.frame_id.compare("ptam") == 0) {
		TooN::SO3<double> res;
//	tf::Quaternion delta_rot = tf::Quaternion(msg.px->orientation.x,
//			msg.px->orientation.y, msg.px->orientation.z,
//			msg.px->orientation.w);
		quaternionToRotationMatrix((*msg).orientation, res);
		imu_rotation_ = res * imu_rotation_;
//	imu_rotation_ = tf::Matrix3x3(delta_rot) * imu_rotation_;
	}
}

template<class T>
bool System::findClosest(const ros::Time & timestamp, std::queue<T> & queue,
		T * obj, const double & max_delay) {
	double best_dt(1e9);
	double tmp_dt;
...
bool System::transformPoint(const std::string & target_frame,
		const std_msgs::Header & header, const geometry_msgs::Point & _p_in,
		TooN::Vector<3> & _p_out) {
...
void System::publishPoseAndInfo(const std_msgs::Header & header) {
	...
	if (mpTracker->getTrackingQuality() && mpMap->IsGood()) {
		...
		tf::StampedTransform transform_ptam(
				tf::Transform(
						tf::Matrix3x3(r_ptam(0, 0), r_ptam(0, 1), r_ptam(0, 2),
								r_ptam(1, 0), r_ptam(1, 1), r_ptam(1, 2),
								r_ptam(2, 0), r_ptam(2, 1), r_ptam(2, 2)),
						tf::Vector3(t_ptam[0] / scale, t_ptam[1] / scale,
								t_ptam[2] / scale)), header.stamp, frame_id,
				PtamParameters::fixparams().parent_frame);

		//camera in the world frame
		TooN::Matrix<3, 3, double> r_world =
				pose.get_rotation().get_matrix().T();
		TooN::Vector<3, double> t_world = -r_world * pose.get_translation();

		tf::StampedTransform transform_world(
				tf::Transform(
						tf::Matrix3x3(r_world(0, 0), r_world(0, 1),
								r_world(0, 2), r_world(1, 0), r_world(1, 1),
								r_world(1, 2), r_world(2, 0), r_world(2, 1),
								r_world(2, 2)),
						tf::Vector3(t_world[0] / scale, t_world[1] / scale,
								t_world[2] / scale)), header.stamp,
				PtamParameters::fixparams().parent_frame, frame_id);
		...
		if (pub_pose_.getNumSubscribers() > 0
			...
			for (unsigned int i = 0; i < msg_pose->pose.covariance.size(); i++)
				msg_pose->pose.covariance[i] = sqrt(
						fabs(covar_world[i % 6][i / 6]));
...
void System::publishPreviewImage(CVD::Image<CVD::byte> & img,
	...
	if (pub_preview_image_.getNumSubscribers() > 0) {
		...
		// subsample image
		CVD::BasicImage<CVD::byte> img_sub((CVD::byte *) &img_msg->data[0],
				sub_size);
		CVD::halfSample(img, img_sub);

		// set opencv pointer to image
		IplImage * ocv_img = cvCreateImageHeader(
				cvSize(img_sub.size().x, img_sub.size().y), IPL_DEPTH_8U, 1);
		...
		if (drawGrid) {
			for (int i = 0; i < dim0; i++) {
				for (int j = 0; j < dim1 - 1; j++)
					cvLine(ocv_img,
							cvPoint(grid[i][j][0] / 2, grid[i][j][1] / 2),
							cvPoint(grid[i][j + 1][0] / 2,
									grid[i][j + 1][1] / 2), CV_RGB(50, 50, 50));

				for (int j = 0; j < dim1 - 1; j++)
					cvLine(ocv_img,
							cvPoint(grid[j][i][0] / 2, grid[j][i][1] / 2),
							cvPoint(grid[j + 1][i][0] / 2,
									grid[j + 1][i][1] / 2), CV_RGB(50, 50, 50));
			}
		}
		...
		if (drawTrails) {
			...
			for (std::list<Trail>::iterator i = trails.begin();
					i != trails.end(); i++) {
				cvLine(ocv_img,
						cvPoint(LevelZeroPos(i->irCurrentPos.x, level) / 2,
...
bool System::pointcloudservice(ptam_com::PointCloudRequest & req,
	...
	if (mpMap->bGood) {
		...
		resp.pointcloud.row_step = resp.pointcloud.point_step
				* resp.pointcloud.width;
		resp.pointcloud.data.resize(
				resp.pointcloud.row_step * resp.pointcloud.height);
		resp.pointcloud.is_dense = false;

		unsigned char* dat = &(resp.pointcloud.data[0]);
		unsigned int n = 0;
		for (std::vector<MapPoint::Ptr>::iterator it = mpMap->vpPoints.begin();
				it != mpMap->vpPoints.end(); ++it, ++n) {
			if (n > resp.pointcloud.width - 1)
				break;
			MapPoint& p = *(*it);
...
bool System::keyframesservice(ptam_com::KeyFrame_srvRequest & req,
	...
	for (std::vector<KeyFrame::Ptr>::reverse_iterator rit =
			mpMap->vpKeyFrames.rbegin(); rit != mpMap->vpKeyFrames.rend();
			++rit) {
		...
		if (takeKF) {
			...
			tf::Transform transform(
					tf::Matrix3x3(rot(0, 0), rot(0, 1), rot(0, 2), rot(1, 0),
							rot(1, 1), rot(1, 2), rot(2, 0), rot(2, 1),
							rot(2, 2)),
					tf::Vector3(trans[0] / scale, trans[1] / scale,
							trans[2] / scale));
